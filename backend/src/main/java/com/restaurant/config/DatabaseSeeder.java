...
public class DatabaseSeeder {
    ...
    private void seedAdmin(UserRepository userRepository) {
        ...
        boolean hasAnyAdmin = userRepository.findAll().stream()
        ...
        if (userRepository.findByEmail("admin@gmail.com").isEmpty()) {
            ...
            System.out.println("[SEEDER] Создан первичный админ: admin@gmail.com / Admin123");
            System.out.println("[SEEDER] ВАЖНО: смените пароль и назначьте дополнительных менеджеров через интерфейс /admin/users");
        }
    }

    private void seedManager(UserRepository userRepository) {
