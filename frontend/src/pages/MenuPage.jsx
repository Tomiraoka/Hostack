...
function MenuPage() {
  ...
  const { auth, isAuthenticated, isAdmin, isUser } = useAuth();
  const { addItem } = useCart();
  const toast = useToast();

  useEffect(() => {
    loadMenu();
    if (isUser && auth?.userId) {
      loadPopular();
    }
  }, [isUser]);
  ...
  async function loadPopular() {
    try {
      const data = await menuApi.getPopular(auth.userId);
      setPopular(data || []);
    } catch {
  ...
  return (
    <>
      <main className="menu-page">
        ...
        {isUser && popular.length > 0 && (
          <section className="menu-section popular-section">
            <h2><Icon name="bell" size={18} className="icon-inline" />Часто заказываете</h2>
            <p className="section-subtitle">Ваши любимые блюда — в один клик</p>
